...
import { Accessories } from '../api/product';
...
const AccessoriesPage = () => {
  ...
  const selectedProducts = Accessories.slice(
    startIndex,
  ...
  const addToCart = (product) => {
    setCart((prev) =>
      prev.some((item) => item.id === product.id)
        ? prev.map((item) =>
            item.id === product.id
              ? { ...item, qutantity: item.quantity + 1 }
              : item,
          )
  ...
  return (
    <div className="wrapper">
      <div className="mb-5 place-items-center">
        <h1 className="text-[30px] font-bold">Accessories</h1>
        <button
          onClick={goToCart}
        ...
        <div className="container mx-auto flex max-w-[59.375rem] flex-col justify-center">
          ...
          <div className="mt-5 flex justify-center">
            <ReactPaginate
              ...
              nextLabel={'Next'}
              breakLabel={'...'}
              pageCount={Math.ceil(Accessories.length / productsPerPage)}
